...
| Running next lint for Lens compliance                        | Checks React rules only                      | pnpm eslint pnpm eslint "src/\*_/_.{ts,tsx}" from design-system root                      |
| <SplitSection> inside <Container> in <Split>                 | SplitSection must be direct child of Split   | <Split><SplitSection><Container>...</Container></SplitSection></Split>                    |
| alignItems="center", justifyContent="space-between" on Split | Using raw CSS values instead of Lens grammar | Use allowed values from Split.md or restructure with SplitSection sizing + Align + Spacer |
